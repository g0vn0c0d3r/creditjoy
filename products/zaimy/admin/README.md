...
- Для витрины оффер должен хранить короткий набор данных: название предложения, сумма, срок, ставка или ПСК, бейджи и ссылка перехода.
- Расширенные условия оффера: способы получения, способы погашения, требования, документы, важные условия и теги. Их можно редактировать внутри оффера, но они не должны превращать оффер в отдельную SEO-страницу.
...
## Задача 5. Упростить `Контроль данных` в организации

### Скриншоты

| Ориентир: общий блок источников |
| --- |
| ![Ориентир общего блока источников](assets/task-05-data-sources-format-reference.png) |

### Что не так сейчас

- В карточке организации есть крупный блок `Контроль данных` с ручными полями `Источник данных`, `Ссылка на источник`, `Ответственный`, `Проверено` и `Внутренние заметки`.
- Для МФО это выглядит как лишняя настройка: одни и те же источники проверки будут повторяться в каждой организации.
- Непонятно, что выбирать основным источником, хотя для всех МФО набор источников должен быть единым.

### Что предлагаем

- Убрать `Контроль данных` как большой ручной блок из карточки организации.
- Общий подход к проверке вынести на уровень продукта или редакционной политики: короткое описание процесса, регулярное обновление и список источников, как в формате на скриншоте.
- Для МФО использовать общий набор источников: официальный сайт компании, Банк России / государственный реестр МФО, ФНС / ЕГРЮЛ для юридических данных, ФЗ-353 и ФЗ-151 как нормативные источники.
- Дополнительные источники показывать только если они реально используются в материале.
- В карточке организации оставить компактно дату последней проверки и ответственного, если эти данные нужны команде.
- Источники не выбирать заново вручную в каждой организации.
