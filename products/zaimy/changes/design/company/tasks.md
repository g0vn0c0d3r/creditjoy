...
**Есть:** в «Клиенты чаще хвалят» и «Клиенты чаще жалуются» пункты идут просто с новой строки, без маркеров, но со списочным отступом.

**Нужно:** оформить существующие пункты маркированным списком с одинаковыми отступами и интервалами в обеих карточках. Если пункт один — маркер тоже показывать.
...
**Нужно:** тот же маркированный список. Маркеры выровнять под иконкой заголовка; продолжение пункта — под его текстом.

```text
[Иконка] Клиенты чаще хвалят
• Первый пункт,
  продолжение первого пункта.
• Второй пункт.
```

Тексты пунктов сохранить. Дату и пояснение под отзывами оставить обычным абзацем.
